...
class Compiler(object):
    def compile(self, compile_config, src_path, output_dir, time_limit, memory_limit):
        ...
        result = _judger.run(max_cpu_time=time_limit,
                             max_real_time=time_limit * 2,
        ...
        if result["result"] != _judger.RESULT_SUCCESS:
            if os.path.exists(compiler_out):
                with open(compiler_out, encoding="utf-8") as f:
                    ...
                    if error:
                        raise JudgeServerException(error)
            raise JudgeServerException("Compiler runtime error, info: %s" % json.dumps(result))
        else:
            os.remove(compiler_out)
            return exe_path